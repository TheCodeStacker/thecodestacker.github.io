...
export function applyStyles(container) {
	...
	container.querySelectorAll('*').forEach(element => cleanupEvents(element));
	...
	const apply = (selector, className) => {
		container.querySelectorAll(selector).forEach(element => element.className = className);
	};
	...
	apply('a', 'text-blue-600 hover:text-blue-700 underline hover:no-underline transition');
	...
	container.querySelectorAll('p').forEach((paragraph, index) => {
		const isMeta = index < 5 && paragraph.querySelector('strong') && paragraph.textContent.includes(':');
		paragraph.className = isMeta 
	...
	container.querySelectorAll('ol').forEach(orderedList => {
		orderedList.className = 'space-y-3 mb-6 pl-0';
		orderedList.querySelectorAll('li').forEach((listItem, index) => {
			listItem.className = 'pl-10 relative bg-white hover:bg-blue-50 p-4 rounded-lg border border-gray-200 transition';
			
			const number = document.createElement('span');
			number.className = 'absolute left-3 top-4 w-6 h-6 rounded-full bg-blue-600 text-white text-sm font-bold flex items-center justify-center shadow-sm';
			number.textContent = index + 1;
			listItem.insertBefore(number, listItem.firstChild);

			const nestedOrderedList = listItem.querySelector('ol');
			if (nestedOrderedList) {
				nestedOrderedList.className = 'mt-3 space-y-2 pl-0';
				nestedOrderedList.querySelectorAll(':scope > li').forEach((nestedListItem, nestedIndex) => {
					nestedListItem.className = 'pl-8 relative bg-gray-50 p-2 rounded-lg';
					const nestedNumber = document.createElement('span');
					nestedNumber.className = 'absolute left-2 top-2 w-5 h-5 rounded-full bg-blue-500 text-white text-xs font-bold flex items-center justify-center';
					nestedNumber.textContent = `${index + 1}.${nestedIndex + 1}`;
					nestedListItem.insertBefore(nestedNumber, nestedListItem.firstChild);
				});
			}
		});
	});

	container.querySelectorAll('ul').forEach(unorderedList => {
		unorderedList.className = 'space-y-2 mb-6 pl-6';
		unorderedList.querySelectorAll(':scope > li').forEach(listItem => {
			listItem.className = 'text-gray-700 leading-relaxed list-disc marker:text-blue-600';
			
			const nestedUnorderedList = listItem.querySelector('ul');
			if (nestedUnorderedList) {
				nestedUnorderedList.className = 'mt-2 space-y-1 pl-6';
				nestedUnorderedList.querySelectorAll(':scope > li').forEach(nestedListItem => {
					nestedListItem.className = 'text-gray-600 list-circle marker:text-blue-500';
				});
			}
		});
	});
	...
	container.querySelectorAll('pre').forEach(pre => {
		...
		copyButton.className = 'bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded text-xs transition';
		...
		addTrackedEvent(copyButton, 'click', copyHandler);
	...
	container.querySelectorAll('blockquote').forEach(blockquote => {
		...
		blockquote.querySelectorAll('p').forEach(paragraph => {
			paragraph.className = 'text-gray-700 italic mb-2 last:mb-0';
		});
	});
	...
	container.querySelectorAll('tbody tr').forEach((tableRow, index) => {
		tableRow.className = index % 2 === 0 ? 'bg-white hover:bg-gray-50 transition' : 'bg-gray-50 hover:bg-gray-100 transition';
	});

	container.querySelectorAll('dl').forEach(definitionList => {
		definitionList.className = 'space-y-2 mb-6';
		definitionList.querySelectorAll('dt').forEach(definitionTerm => {
			definitionTerm.className = 'font-semibold text-gray-900 text-base';
		});
		definitionList.querySelectorAll('dd').forEach(definitionDescription => {
			definitionDescription.className = 'ml-4 text-gray-700 text-sm';
		});
	});
	...
	container.querySelectorAll('details').forEach(details => {
		...
		if (summary) {
			summary.className = 'cursor-pointer font-semibold text-gray-900 bg-gray-50 px-4 py-3 hover:bg-gray-100 transition';
		}
		
		const content = Array.from(details.children).filter(element => element.tagName !== 'SUMMARY');
		content.forEach(element => {
			if (element.tagName === 'P') {
				element.classList.add('px-4', 'py-3');
			}
		});
	});
	...
	container.querySelectorAll('h1, h2, h3, h4, h5, h6').forEach(heading => {
		heading.id = heading.textContent.toLowerCase().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-');
	});

	backToTopButton = document.createElement('button');
	backToTopButton.className = 'fixed bottom-6 right-6 bg-blue-600 hover:bg-blue-700 text-white w-12 h-12 rounded-full shadow-lg transition z-40 opacity-0 pointer-events-none flex items-center justify-center text-xl font-bold hover:scale-110';
