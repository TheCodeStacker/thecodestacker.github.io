...
async function renderSidebarContent() {
	...
	document.querySelectorAll('.group-toggle').forEach(button => {
		...
		eventHandlers.groupToggleHandlers.push({ element: button, handler });
...
export async function renderSidebar() {
	...
	const render = async () => {
		...
		element.innerHTML = `
			...
			<div id="sidebar-overlay" class="fixed inset-0 bg-black/50 z-30 lg:hidden hidden transition"></div>
			<div id="sidebar-content" class="fixed lg:sticky top-0 left-0 z-40 w-64 bg-white border-r border-gray-200 h-screen flex flex-col transform -translate-x-full lg:translate-x-0 transition shadow-xl">
		...
		if (!toggle || !close || !overlay || !header) return;
		...
		eventHandlers.languageHandler = (event) => {
			if (event.target.closest('#language-button')) {
				openLanguageModal();
			}
		};
		element.addEventListener('click', eventHandlers.languageHandler);
